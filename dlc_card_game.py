...
# ---------- Helpers to load images as data-URI ----------
...
/* Header: logo at left, text block at right (title + subtitle) */
.header {{
  display: grid;
...
.header .textblock .accent, .header .textblock b {{
  color: var(--accent);
  font-weight: 800;
}}
...
.card.face-down .icon img {{
  opacity: 0.9; filter: grayscale(100%) brightness(0) saturate(100%) sepia(0) hue-rotate(0deg);
}}
